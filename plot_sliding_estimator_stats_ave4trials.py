...
a_vs_bs = ['stim-face_vs_stim-house']
scores = {'stim-face_vs_stim-house': list()}
for subject_id in range(1, 26):
    ...
    for a_vs_b in a_vs_bs:
        fname_td = os.path.join(data_path, '%s-causal-highpass-2Hz-td-auc-%s-ave4trials.mat'
                                % (subject, a_vs_b))
...
fname_stats = os.path.join(data_path, 'stats-causal-highpass-2Hz-td-auc-%s-ave4trials.mat'
                                % a_vs_b)
...
title = 'Decoding Averaged 4 trials %s (p < 0.001)'%a_vs_b
ax.set(title=title)
...
fig.savefig(os.path.join('/home/claire/DATA/Data_Face_House_new_proc/Analysis/Figures', 'stat_time_decoding_highpass-2Hz-%s-ave4trials.pdf' %a_vs_b), bbox_to_inches='tight')
